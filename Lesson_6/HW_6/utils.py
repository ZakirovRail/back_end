import json
import os
import sys
import logging

logger_server = logging.getLogger('server_app')
logger_client = logging.getLogger('client_app')


def load_configs(is_server=True):
    config_keys = [
        "DEFAULT_IP_ADDRESS",
        "DEFAULT_PORT",
        "MAX_CONNECTIONS",
        "MAX_PACKAGE_LENGTH",
        "ENCODING",
        "ACTION",
        "TIME",
        "USER",
        "ACCOUNT_NAME",
        "PRESENCE",
        "RESPONSE",
        "ERROR"
    ]
    if not is_server:
        config_keys.append('DEFAULT_IP_ADDRESS')
    if not os.path.exists('config.json'):
        print('The configuration file was not found')
        sys.exit(1)
    with open('config.json') as config_file:
        CONFIGS = json.load(config_file)
    loaded_configs_keys = list(CONFIGS.keys())
    for key in config_keys:
        if key not in loaded_configs_keys:
            print(f'Missed key {key} in the configuration file')
            sys.exit(1)
    return CONFIGS


def send_message(opened_socket, message, CONFIGS):
    json_message = json.dumps(message)
    response = json_message.encode(CONFIGS.get('ENCODING'))
    opened_socket.send(response)


def get_message(opened_socket, CONFIGS):
    response = opened_socket.recv(CONFIGS.get('MAX_PACKAGE_LENGTH'))
    if isinstance(response, bytes):
        json_response = response.decode(CONFIGS.get("ENCODING"))
        response_dict = json.loads(json_response)
        if isinstance(response_dict, dict):
            return response_dict
        raise ValueError
    raise ValueError


class Log:
    def __call__(self, func):
        def decorator(*args, **kwargs):
            file_name = os.path.basename(sys.argv[0])
            return_func = func(*args, **kwargs)
            if 'server.py' in file_name:
                print(file_name)
                logger_server.debug(f'The function {func} were called from the main() \n'
                                    f'The module is: {func.__module__} \n '
                                    f'The file name is {os.path.basename(sys.argv[0])}')
                # return return_func
            elif 'client.py' in file_name:
                print(file_name)
                logger_server.debug(f'The function {func} were called from the main()\n'
                                    f'The module is: {func.__module__} \n'
                                    f'The file name is {os.path.basename(sys.argv[0])}')
                # return return_func
            else:
                raise ValueError
            return return_func
        return decorator


# def log(func):
#     def decorator(*args, **kwargs):
#         file_name = os.path.basename(sys.argv[0])
#         return_func = func(*args, **kwargs)
#         if file_name == 'server.py':
#             logger_server.debug(f'The function {func} were called from the main()')
#             return return_func
#
#         elif file_name == 'client.py':
#             logger_server.debug(f'The function {func} were called from the main()')
#             return return_func
#     return decorator
